<template>
  <div>
    ...
    <el-row>
      <el-col>
        <el-table class="tablelist" v-loading="loading" :data="Table.tableData_prepag" style="width: 100%;position:relative;" :height="this.Table.height">
          <el-table-column v-for="title in Table.tableData_title" v-if="title.name!=='total'" :prop="title.name" :render-header="renderHeader"
            :formatter="contentFormatter" :min-width="title.width" align="center" header-align="center" :resizable="false"></el-table-column>
        </el-table>
      ...
      <el-dialog size="mini" title="热搜核心词" :visible.sync="dialogFormVisible">
        <el-form :model="ruleForm" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          ...
          <el-form-item label="搜索人气">
            <el-col :span="4">
              <el-form-item prop="v1m" :rules="[{type: 'number',message: '人气值为数值',trigger: 'change'}]">
                <el-input v-model.number="ruleForm.v1m" auto-complete="off"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="2" style="text-align:center">—</el-col>
            <el-col :span="4">
              <el-form-item prop="v1l" :rules="[{ type: 'number',message: '人气值为数值',trigger: 'change'}]">
          ...
          <el-form-item label="点击率">
           ...
           <el-form-item label="点击人气">
              <el-col :span="4">
              <el-form-item prop="v4m" :rules="[{type: 'number',message: '转化率指数必须为数字值',trigger: 'change'}]">
                <el-input v-model.number="ruleForm.v4m" auto-complete="off"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="2" style="text-align:center">—</el-col>
            <el-col :span="4">
              <el-form-item prop="v4l" :rules="[{ type: 'number',message: '转化率指数必须为数字值',trigger: 'change'}]">
    ...
    methods: {
      ...
      objToArr(obj) {
        ...
        if (obj) {
          ...
          this.Table.tableData_prepag = middle_Table_body //每页默认取出20条数据
        }
      },
      submitForm(formName) {
          ...
          if (valid) {
            for (var ite in this.ruleForm) {
              ...
              if (!idhas && this.ruleForm[ite] != '' && this.ruleForm[ite] != null) {
                if (ite == 'titler') {
                  this.dynamicTags.push('搜索词：' + this.ruleForm[ite])
                } else if (ite == 'v1m') {
                  this.dynamicTags.push('人气高于：' + this.ruleForm[ite])
                } else if (ite == 'v1l') {
                  this.dynamicTags.push('人气低于：' + this.ruleForm[ite])
                } else if (ite == 'v2m') {
                  this.dynamicTags.push('词数高于：' + this.ruleForm[ite])
                } else if (ite == 'v2l') {
                  this.dynamicTags.push('词数低于：' + this.ruleForm[ite])
                } else if (ite == 'v3m') {
                  this.dynamicTags.push('点击率高于：' + this.ruleForm[ite])
                } else if (ite == 'v3l') {
                  this.dynamicTags.push('点击率低于：' + this.ruleForm[ite])
                }else if (ite == 'v4m') {
                  this.dynamicTags.push('点击人气高于：' + this.ruleForm[ite])
                } else if (ite == 'v4l') {
                  this.dynamicTags.push('点击人气低于：' + this.ruleForm[ite])
                }else if (ite == 'v5m') {
